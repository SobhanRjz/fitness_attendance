...
A Laravel REST API for gyms to manage class attendance: list attendees, toggle one
attendee, or toggle the whole class — safely, even with multiple staff editing the
same class at once.

> **Status:** Backend is fully implemented and tested. The React Native attendance
> roster screen is implemented and connected to it — see [Frontend](#frontend-react-native).
...
│   ├── Exceptions/       # AttendanceConflictException
│   ├── Http/
...
└── tests/
    ├── Feature/Attendance/   # Endpoint tests
    └── Unit/Services/        # Business logic tests
```

Flow: **Controller → Form Request → Service → Model → Resource** — each layer has one job.
...
| **"Mark all" skips version checks** | Requiring 20+ version numbers per bulk request (and failing the whole batch if one is stale) doesn't match staff intent. It's a deliberate overwrite — but it still bumps every row's version, so a stale single-row update sent afterward is still rejected. |
...
| **Conflict handled as a custom exception, rendered centrally** | Keeps the controller's happy path clean; `409` responses carry the current row so the client can resync instantly. |

## Assumptions

1. **PostgreSQL** was chosen as a common Laravel production default; the schema works
   unmodified on MySQL.
2. **Lost updates are unacceptable; a `409` is better than silent data loss.** The
   assignment explicitly calls out concurrent toggles, so this was treated as a hard
   requirement, not an edge case. "Mark all" is the one intentional exception.
3. **No auth layer** — the frontend spec says it's not required. `authorize()` in both
   Form Requests returns `true` with a note on where access control would go.
4. **Multi-tenancy exists in the schema** (`Gym` owns `Member`/`FitnessClass`) but
   isn't enforced at the API layer, since there's no authenticated staff user yet.
5. **`marked_at`** reflects only the latest change (set on attended, cleared on
   not-attended) — not a full history log. A real audit trail would be a separate table.
6. **Members must already be enrolled** (have an attendance row) to be marked — the
...
# update DB_* in .env, then:
createdb fitness_attendance
php artisan migrate
php artisan serve
```
...
{
  "data": [
    { "id": 1, "member": { "id": 10, "name": "Jane Doe" }, "status": "not_attended", "version": 1, "marked_at": null }
  ]
}
```
...
| `404` | Member not enrolled in this class. |
...
- **Feature tests** — real HTTP requests: list, single update (incl. `409`/`404`), bulk update.
- **Unit tests** — `AttendanceService` directly: status transitions, version bumps, conflict handling.

## Frontend (React Native)

An Expo app implementing a single screen: a live class attendance roster, matching the
provided design's three states (loaded / loading / error). Structure follows
`.cursor/rules/react-native.mdc` (dedicated API layer, hooks own data-fetching, small
...
└── features/attendance/
    ├── api/attendanceApi.ts            # fetch calls + typed errors (AttendanceApiError, AttendanceConflictError)
    ├── hooks/useAttendance.ts          # Load state, optimistic toggle + rollback, mark-all
    ├── utils/avatar.ts, membership.ts  # Deterministic display-only placeholders (see Assumptions)
    ├── components/                     # RosterTopBar, ClassSummaryCard, AttendeeRow, RosterSkeleton, RosterErrorState, ...
...
[Assumptions](#assumptions) below for why, and for the tradeoff considered.
...
`http://10.0.2.2:8000/api` (Android emulator) — both map to the backend running on your
...
Unit tests cover `useAttendance` (optimistic toggle, rollback on failure, `409`
reconciliation, mark-all) and the avatar/membership placeholder utilities.
